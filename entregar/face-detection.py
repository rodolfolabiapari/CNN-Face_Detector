...
def main():
    ...
    if CONST_train_again == "y":
        train_set = cnn_functs.loading_set_for_training(num_files_94, l_directories_fddb_train,
                                                        CONST_size_image_algorithm)

        test_set, figure_test_set = cnn_functs.loading_set_for_testing(l_directories_fddb_test, CONST_batch_size,
                                                                       CONST_size_image_algorithm)

        model = cnn_functs.train_model_fddb_94(train_set, test_set, CONST_num_epochs, CONST_learning_rate,
                                               CONST_momentum)
        ...
        if os.path.exists("./cnn/cnn-trained_model-" + str(num_files_94) + "-" + str(len(l_directories_fddb_train)) +
                                  "-" + str(CONST_size_image_algorithm) + "-" + str(CONST_num_epochs) + ".prm"):

            model = Model("./cnn/cnn-trained_model-" + str(num_files_94) + "-" + str(len(l_directories_fddb_train)) +
                          "-" + str(CONST_size_image_algorithm) + "-" + str(CONST_num_epochs) + ".prm")
        else:
    ...
    for list_images in l_directories_fddb_valid:
        valid_set, figure_valid_set = cnn_functs.loading_set_for_validation(list_images, CONST_batch_size,
                                                                            CONST_size_image_algorithm)
    ...
    print "\tTime spend to organize: ", (end_program - start_program) / \
                                        60, 'minutes, or ', ((end_program - start_program) / 60) / 60, " hours\n"
